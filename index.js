...
async function generateJwtToken(clientId, clientSecret, refreshToken) {
...
async function updatePackage(extId, zipPath, token) {
    let url;
    let headers;
    let response;
    let uploadState;
    ...
    const body = fs_1.default.createReadStream(path_1.default.resolve(zipPath));
    headers = { Authorization: `Bearer ${token}`, 'x-goog-api-version': '2' };
    response = await axios_1.default.put(url, body, { headers, maxContentLength: Infinity });
    ...
    while (uploadState === 'IN_PROGRESS') {
        ...
        response = await (0, axios_1.default)(url, headers);
        uploadState = response.data.uploadState;
    }
...
async function publishExtension(extId, testerOnly, token) {
    ...
    const response = await axios_1.default.post(url, null, { headers, params: { target } });
    core.debug('Response status code: ' + response.status);
    core.debug(JSON.stringify(response.data));
...
async function run(extensionId, zipPath, testerOnly, clientId, clientSecret, refreshToken) {
